(function() {
    ...
    function setEnumerationUnits(caliCounties, californiacenters, CAmap, path) {
        ...
        var centroids = CAmap.selectAll(".symbol14")
            .data(californiacenters.features.sort(function(a, b) {
                return b.properties.measles14 - a.properties.measles14;
            }))
            .enter().append("path")
            .attr("class", function(d) {
            ...
            .attr("d", path.pointRadius(function(d) {
                return radius(d.properties.measles14);
            }))
            .style({
                "fill": "#fc8d59",
                "fill-opacity": 0.5,
            ...
            .on("mouseover", function(d) {
                tooltip.style("visibility", "visible").html("<l1>" + labelTitles2.measles14 + ":   " + "<b>" + d.properties.measles14 + "</b><div>" + "County: " + "<b>" + d.properties.county.replace(/([A-Z])/g, ' $1').trim() + "</b></div></l1>");
    ...
    function selectLayer(caliCounties, californiacenters, dataMeasles, CAmap, path) {
        let vaccineCoverageLegendDisplay = document.querySelector('.VClegendContainer');
        let personalBeliefLegendDisplay = document.querySelector('.PBElegendContainer');

        d3.selectAll('.radio').on('change', function() {
            if (document.getElementById('propsymbs14').checked) {
              vaccineCoverageLegendDisplay.style.display = 'none';
              personalBeliefLegendDisplay.style.display = 'none';
            ...
            if (document.getElementById('vc13').checked) {
                vaccineCoverageLegendDisplay.style.display = 'flex';
                personalBeliefLegendDisplay.style.display = 'none';
                ...
                var counites = d3.selectAll('.counties').transition().duration(200)
                    .style('fill', function(d) {
                        return colorScaleVC(d.properties.coverage1314)
                    })
                    .style('stroke', 'white')
              ...
              vaccineCoverageLegendDisplay.style.display = 'none';
              personalBeliefLegendDisplay.style.display = 'flex';

                CAmap.selectAll('.circle13').remove();
                CAmap.selectAll('.circle14').remove();
                d3.selectAll('.counties').transition().duration(200)
                    .style('fill', function(d) {
                        return colorScalepb13(d.properties.pbe1314)
                    })
                    .style('stroke', 'white')
                var singleCounties = CAmap.selectAll(".counties").data(caliCounties)
                    .on('mouseover', function(d) {
                    ...
                    .on('mousemove', function() {
                        return tooltip.style("top", (event.pageY - 40) + "px").style("left", (event.pageX + 40) + "px");
                    })
                    .on('mouseout', function(d) {
                        tooltip.style("visibility", "hidden");
                        dehighlight(d.properties)
              ...
              personalBeliefLegendDisplay.style.display = 'none';
                ...
                d3.selectAll('.counties').transition().duration(200)
                    .style('fill', function(d) {
                        return colorScaleVC(d.properties.coverage1516)
                    })
                    .style('stroke', 'white');
                var singleCounties = CAmap.selectAll(".counties").data(caliCounties)
                    .on('mouseover', function(d) {
              ...
              personalBeliefLegendDisplay.style.display = 'flex';
                ...
                d3.selectAll('.counties').transition().duration(200)
                    .style('fill', function(d) {
                        return colorScalepb14(d.properties.pbe1516)
                    })
                    .style('stroke', 'white');
                var singleCounties = CAmap.selectAll(".counties").data(caliCounties)
                    .on('mouseover', function(d) {
              ...
              personalBeliefLegendDisplay.style.display = 'none';
                ...
                var centroids = CAmap.selectAll(".symbol13")
                    .data(californiacenters.features.sort(function(a, b) {
                        return b.properties.measles13 - a.properties.measles13;
                    }))
                    .enter().append("path")
                    .attr("class", function(d) {
                    ...
                    .attr("d", path.pointRadius(function(d) {
                        return radius(d.properties.measles13);
                    }))
                    .style({
                        "fill": "#fc8d59",
                        "fill-opacity": 0.5,
                    ...
                    .on("mouseover", function(d) {
                        tooltip.style("visibility", "visible").html("<l1>" + labelTitles2.measles13 + ":   " + "<b>" + d.properties.measles13 + "</b><div>" + "County: " + "<b>" + d.properties.county.replace(/([A-Z])/g, ' $1').trim() + "</b></div></l1>");
                        highlightCircles(d.properties)
                    })
                    .on("mousemove", function() {
                        return tooltip.style("top", (event.pageY - 50) + "px").style("left", (event.pageX + 50) + "px");
